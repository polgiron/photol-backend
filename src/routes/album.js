...
router.get('/all', authGuard, async (req, res) => {
  await req.context.models.Album.find(
    {
    ...
    .exec((err, albums) => {
      ...
      albums.forEach((album) => {
        if (album.cover) {
          album.cover.signedUrl = getSignedUrl(
            album.cover,
            req.payload.email,
            'small'
          )
...
router.get('/:albumId', authGuard, async (req, res) => {
  await req.context.models.Album.findById(req.params.albumId)
    .populate('cover')
    .populate({
      path: 'images',
      options: {
...
router.put('/:albumId', authGuard, async (req, res) => {
  await req.context.models.Album.findByIdAndUpdate(
    req.params.albumId,
    req.body,
    {
      new: true
...
router.delete('/:albumId', authGuard, async (req, res) => {
  await req.context.models.Album.findByIdAndRemove(
    req.params.albumId,
    (err, album) => {
      if (err) return res.status(500).send(err)
